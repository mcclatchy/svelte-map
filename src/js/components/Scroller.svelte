<script context="module">
  import { isMobile, isTablet } from '../modules/utils.js';
	...
	wh = isMobile.ios() || isTablet.ipad() ? outerHeight : innerHeight; 
	...
	if (typeof IntersectionObserver !== 'undefined') {
		...
		manager = {
			add: ({ outer, update }) => {
				...
				if (top < wh && bottom > 0) handlers.push(update);
...
<script>
	...
	function update() {
		...
		if (print) {
			console.log("Scroller", progress, top_px, fg.top, foreground_height, available_space)
		}
		if (progress <= 0) {
			offset_top = 0;
			fixed = false;
...
<svelte:window bind:innerHeight={innerHeight} bind:outerHeight={outerHeight}/>
